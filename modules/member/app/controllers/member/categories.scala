...
@Singleton
class ArticleCategories @Inject()(cc: ControllerComponents) extends AbstractController(cc) with play.api.i18n.I18nSupport {
  object FormModel{
  ...
  def list = Action  { implicit request =>
    //Ok(views.html.blogs.article_categories.list())
    Ok("")
  }

  def create = Action  { implicit request =>
    Ok("")
    //Ok(views.html.blogs.article_categories.create())
  }

  def edit(id:Long) = Action  { implicit request =>
    Ok("")
    //Ok(views.html.blogs.article_categories.edit(ArticleCategory.findById(id).get))
  }

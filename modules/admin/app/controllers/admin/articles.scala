...
import modules.uploads.UploadComponents
...
class Articles @Inject()(flashingCache:FlashingCache,
  ...
  case class ArticleData(code:Option[String],
    ...
    def toEntity = {
      updateEntity(new Article())
    }

    def updateEntity(article:Article) = {
      article.title = title
  ...
  def publishNew() = Action { implicit request =>
    ...
    articleForm.bindFromRequest.fold(
      ...
      model => {
        ...
        entity.createdBy = request.session.get("loginUser").get
        entity.publishNew
        render {
          case Accepts.Json() => {
    ...
    articleForm.bindFromRequest.fold(
      ...
      data => {
        findArticle.map{ article => {
          ...
          data.updateEntity(article)
          article.updatedBy = request.session.get("loginUser").get
  ...
  def editormdSave() = checkLoginAction { implicit request =>

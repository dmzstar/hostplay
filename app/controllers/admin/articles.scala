package controllers.admin.articles
...
import controllers.{FlashingCache, routes, secs}
...
@Singleton
class ArticleController @Inject()(flashingCache:FlashingCache,
  ...
  import views.html.admin.{articles => Views}
  ...
  def create()(implicit form:Form[ArticleData] = articleForm) = Action { implicit request =>
    ...
    val result = Ok(Views.create(form,ArticleCategory.query.findList()))
    Conversation.cleanLast(result)
